...
class CaptureChannel {
 public:
  ...
  // The editor host the direct-capture flow's open-in-editor leg reveals (set
  // once by FlutterWindow).
  void SetEditorHost(EditorHostClient* editor) { editor_host_ = editor; }
  // A capture saved a file into the shared recent store (set once by
  // FlutterWindow; fans out to the tray list + a live editor).
  void SetRecentChangedCallback(std::function<void()> cb) {
    recent_changed_cb_ = std::move(cb);
  }
 ...
 private:
  ...
  EditorHostClient* editor_host_ = nullptr;    // not owned
  std::function<void()> recent_changed_cb_;
