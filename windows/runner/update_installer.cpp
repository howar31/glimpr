...
// The release-signing PUBLIC key (Ed25519, raw 32 bytes). CI signs
// Glimpr-Setup.exe with the matching private key (the RELEASE_SIGNING_KEY
// repo secret) and publishes the detached signature as Glimpr-Setup.exe.sig.
// An all-zero key verifies nothing, so until provisioning replaces it the
// updater fails closed to the release-page fallback.
