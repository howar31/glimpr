...
class Checkerboard extends StatelessWidget {
  const Checkerboard({super.key, this.tile = 12, required this.dark});
...
class _CheckerPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    // Two-tone grid: a flat base fill, then the darker squares painted over it
    // on alternating cells. Greys are appearance-matched so the board recedes.
    final base = dark ? const Color(0xFF20242C) : const Color(0xFFE9ECF1);
    final alt = dark ? const Color(0xFF181B22) : const Color(0xFFD7DCE4);
