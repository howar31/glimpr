...
/// in memory PER ENGINE INSTANCE and only refreshes on an explicit reload. A
/// resident overlay engine therefore serves settings frozen at its first read and
/// never sees the control (settings) engine's later writes -- so decoration
/// toggles, loupe size/zoom, etc. silently fall back to defaults in the overlay.
/// Reloading the cache before the overlay reads its per-capture config restores
/// the macOS behavior. Best-effort: any failure leaves the existing cached values
/// (a stale read is acceptable; a crash is not).
Future<void> reloadSettingsForOverlay() async {
