...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/media/gif-editor-mac-zh.png">
  <source media="(prefers-color-scheme: light)" srcset="docs/media/gif-editor-mac-zh-light.png">
  <img alt="Glimpr GIF 編輯" src="docs/media/gif-editor-mac-zh.png">
</picture>

在圖片編輯器開啟任何 GIF，或剛錄完的 GIF 錄影，編輯器會切換成幀時間軸讓你逐幀編輯：

- **幀時間軸**：選取、刪除、刪除選取範圍之前或之後的全部幀、排序、反轉、來回循環；移除重複幀；減少幀數；幀剪貼簿；完整復原／重做。
- **時間**：設定、增減或縮放幀延遲；插入標題幀。
- **畫布**：裁切、調整大小、翻轉、旋轉、邊框。
- **標註**：把圖片編輯器的工具（形狀、文字、模糊、像素化、聚光燈等）燒入指定的幀範圍。
- **動態**：淡出與滑動轉場、一鍵平滑循環、局部動態與進度列。
- **輸出**：自適應調色盤、可選抖動、檔案大小最佳化（幀差分）與循環次數，全部由 Glimpr 自家編碼器重新編碼。

入口：在圖片編輯器（選單列／系統匣選單，或全域快捷鍵 `⌘⌥9`／`Ctrl+Alt+Win+9`）開啟或拖放任何 `.gif`、點選最近項目中的 GIF，或在「設定 > 錄影」開啟錄影完成後的流程動作，錄完直接進入編輯。
...
安裝程式預設只安裝給目前帳號，不需要系統管理員權限；安裝時可選「所有帳號」，
之後也可在「設定 > 進階」切換。受管理的電腦可在命令列以 `/ALLUSERS` 或
`/CURRENTUSER` 執行安裝程式（請先結束 Glimpr）。

已安裝的版本會自我更新：有新版本時，從選單列（系統匣）選單或「關於」頁面一鍵完成更新。僅此帳號的安裝更新時不會出現系統管理員核准。
