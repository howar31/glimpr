...
- **GIF 編輯器**：逐幀編輯任何 GIF 並重新編碼成更小的檔案，詳見下方 [GIF 編輯器](#gif-編輯器)。
...
<picture>
  ...
  <img alt="Glimpr GIF 編輯器" src="docs/media/gif-editor-mac-zh.png">
</picture>

開啟任何 GIF，或剛錄完的 GIF 錄影，逐幀編輯：

- **幀時間軸**：選取、刪除、排序、反轉、來回循環；移除重複幀；減少幀數；幀剪貼簿；完整復原／重做。
...
入口：選單列（系統匣）選單、全域快捷鍵（`⌘⌥⇧9`／`Ctrl+Alt+Win+Shift+9`）、開啟或拖放任何 `.gif` 到編輯器視窗，或在「設定 > 錄影」開啟錄影完成後的流程動作，錄完直接進入編輯。
